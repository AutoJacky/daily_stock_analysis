...
- [修复] 申万官方一级行业接口在 GitHub 海外节点断连或返回非 JSON 时，15 秒硬超时后自动改用新浪公开的完整 31 个申万2021一级行业节点，以同交易日成分股前一日流通市值加权收益生成明确标注的代理排名；缺行业、覆盖率低于80%或交易日不一致仍阻断正式结论，禁止把东财自定义板块或代理值冒充申万官方指数。
- [新功能] 多源融合终稿支持加密个人组合操作单：公开基金逐日复权收益按动量、波动、Sharpe 与回撤排序，使用周度滚动样本外的前后20%真实涨跌差诊断因子质量，并结合美元固收、基金、黄金和未知现金形成具体减持金额与净值触发条件；明文持仓不进入公开仓库、千问请求、Actions artifact 或日志。
- [修复] 多源融合机构数据按严格市场底稿的完整交易日截断，不再在美股盘中混入下一交易日的 VIX、汇率或商品快照；美股终稿隐藏 A 股专属估值/北向/融资栏目，并正确提取“加风险条件/减风险条件”。
...
- [改进] 多源融合报告升级为机构级证据框架，新增指数PE/PB五年分位、ERP代理、全球联动、融资余额、北向披露状态、行业估值、个股基本面缺口和无伪概率情景；所有字段保留来源、日期与状态，免费源未覆盖项明确标记而不补造。
- [新功能] 支持把千问客户端两个定时任务保存的当日Markdown通过本机桥接写入GitHub Encrypted Secret，云端按市场与日期验证后作为第三观点源参与融合，报告正文不进入公开Git历史且不读取千问登录数据库。
- [文档] 新增A股盘后与美股科技半导体两条千问客户端任务的完整可复制提示词、建议时刻和首次落盘验证步骤。
- [修复] 魔搭免费千问容量不足或返回空结果时不再造成当日微信报告静默缺席，改为明确标记审计未完成并继续发送程序校验的机构框架报告，且仍禁止任何收费回退。
- [修复] 融资余额统一按人民币亿元换算和展示，避免十亿元数值被误标为亿元造成10倍单位偏差。

- [修复] 美股 ETF 批量行情在 GitHub Runner 偶发漏回或落后单个行业时，按缺失/落后标的逐一补采并重用完整交易日截断校验，避免行业 ETF 不足 11/11 导致整份美股收盘报告被误阻断。
- [修复] A 股多源指数合并改为交易日优先，禁止字段较多但落后一日的历史源覆盖当日腾讯收盘；落后一日的沪/深成交额仅用作前日对比证据，并新增六大指数同日强制校验。
- [修复] A 股指数备用源仅返回单日行情时不再把当日收盘冒充昨收并显示为 0.00%；Yahoo 指数窗口扩大到 5 日且必须取得两个交易日，所有指数涨跌额/幅在报告边界再用现价与昨收强制复算。
- [修复] A 股收盘复盘改用腾讯六指数同日收盘快照与 Baostock 历史成交额跨源校验，东财断连时仍可恢复上证/深证前一交易日成交额；两市额统一取上证综指+深证成指，并显示放量/缩量额与比例。
- [修复] A 股行业排名改为仅接受申万一级分类，拒绝将国民经济行业或东财自定义板块冒充券商行业口径；盘中宽度与上一完整交易日指数日期不一致时直接阻断结论。
- [修复] 美股复盘的指数、SPY/RSP/IWM/QQQ、11 个行业 ETF 和 FRED/美国财政部利率统一截断至最近完整交易日，新增指数/ETF 幅度交叉验证与 SPY 至少 15 个历史成交量观测门槛，禁止把盘中数据标成当日收盘。
- [改进] 复盘固定评分加入成交额环比和权重/成长分化维度，压低仅由涨停家数推高的过度乐观分数；加仓规则改为收盘位、宽度和预估成交额共同确认，不再要求精确突破当日最高点。
- [改进] 微信手机版指数表收窄为收盘/昨收/涨跌幅/成交额核心列，OHLC 改为独立审计行；报告与结构化载荷同步附带数据源、抓取时间、数据日、前日成交额及环比。
- [测试] 新增 A 股指数断连补抓、腾讯/Baostock 跨源合并、前日成交额排重、宽度交易日对齐、申万分类护栏以及美股未收盘截断回归测试。
- [修复] 微信个股决策卡不再把境外市场不适用的 A 股式主力资金流误标为“数据不足”；改由确定性正式推送门槛判定核心证据，并将核心证据完整性与模型置信度分开展示。
- [修复] 每日分析报告固定只经 PushPlus 投递个人微信，并让飞书会话回复与云文档同样遵守 report 路由。
- [改进] 个股和大盘数据质量复盘升级为递进式自愈闭环，自动执行缓存清理、多源补采、恢复短期熔断、重建大盘数据管理器、重新生成及证据校验。
...
- [修复] 大盘复盘区分行情缺失与真实零值：可从上证指数和深证成指恢复两市成交额；核心指数或市场宽度校验失败时跳过 LLM、情绪温度及仓位/交易建议，改为输出确定性数据异常报告。
- [修复] PushPlus 在多股票长报告占满每分钟请求额度后会丢失大盘复盘：默认非合并完整流程改为优先生成并发送大盘，区分“已生成”和“已送达”并在失败后重试；发送器增加跨消息的五次/分钟滚动保护与服务端限流重试，并为大盘、模型复核和自选股使用可区分的短标题。
- [改进] PushPlus 改用 HTTPS 与手机优先 HTML：正文采用 16px 卡片式排版、彩色结论/风险/数据缺口、自适应表格和隐藏式来源链接；34 股推送改为最多两页的重点卡片加全量成功/失败清单，重点股票直接附可核验公司动态，完整证据版仍保存到 Actions 附件。
- [新功能] GitHub Actions 按定时/手动运行隔离历史数据库，只持久化通过完整性检查的定时样本；每日自动执行长周期回测及 1/3/5/10 交易日信号后验，逐股票样本达标后 Agent memory 仅可下调置信度，并推送不含模型自评的客观预测复核摘要。
- [新功能] 每日后验新增版本化自适应治理快照：按样本量、数据不可评估率、方向命中率和模拟收益进入采样、阻断、限制、谨慎或稳定状态；下一轮只允许维持或降低置信度，达标策略仅进入影子候选，真实券商下单权限永久关闭。
- [改进] PushPlus 微信报告升级为新手友好的 PPT 卡片式摘要：先给一分钟结论，再展示当前动作、明日观察、风险失效和数据可信度，并按风险/机会/等待分组全部自选；高对比语义色块和自动术语小抄只改善阅读，不改写原始数据或删除完整证据版。
- [新功能] 美股大盘复盘升级为与 A 股同级的严格证据链：同日校验 6 个指数、SPY/RSP/IWM/QQQ 参与度、11 个标普行业 ETF、SPY 量比、FRED 美债/美元及带来源政策政治财报新闻；核心层缺失即禁止方向和仓位结论。
- [修复] 美股严格复盘新增美国财政部官方每日收益率曲线作为 FRED 美债备用源，并按报告交易日截断，避免云端网络差异造成误缺失或读取未来数据。
- [修复] 美股大盘报告启用强制事实审计：正文改由确定性模板渲染，禁止模型补写未检索到的财报、政策、行业原因和关键价位；宏观表逐项显示 FRED 或美国财政部真实来源。
- [改进] GitHub Actions 拆分 A 股与美股收盘任务：北京时间 15:10 推送 A 股完整分析，21:15 UTC 仅推美股大盘复盘，交易日按各自市场时区校验，避免未收盘或重复报告。
- [测试] 新增美股 ETF/FRED 数据标准化、交易日对齐、严格降级、多因子 Prompt 和独立云端调度回归测试。
- [修复] A 股市场宽度供应商增加可配置的单源硬超时与自动换源，防止外部接口卡死后同时丢失 A 股大盘复盘和全部自选分析。
- [改进] GitHub Actions 的 A 股/美股收盘批次共用一份自选但按市场自动分流；两个批次都会先推送大盘复盘，再推送同市场个股决策卡。
- [改进] PushPlus 个股报告区分 A 股/美股标题，显示持有者与未持有者的条件加仓、持有/观望、减仓/回避动作，并用高对比色块标注行动、风险和数据质量。
- [新功能] 日股代码支持 JPX 2024 年起含字母的四位证券代码（如铠侠 `285A.T`），贯通 WebUI 校验、自选 API、市场识别、YFinance 路由与中英文资讯别名。
- [改进] 新增北京时间 16:15 的港股/日股自选收盘批次，与 A 股、美股批次共用同一份云端自选并按市场去重分流。
- [修复] 正式定时分析取消 30 分钟/单 worker 的容量瓶颈，改为 180 分钟、3 worker 和 LiteLLM 429/5xx/超时的可配置指数退避；硬配额或余额不足仍立即失败，不伪造绕过上游限额。
- [修复] PushPlus 改按官方“字符”口径校验长度，个股和大盘报告超限时在彩色 HTML/精简 HTML/单条 Markdown 间自动瘦身，不再分页为多条微信；单股标题固定包含名称和代码。
- [改进] 个股正式推送新增最低证据契约，行情、MA5/10/20、财报、带链接的可核验事件和持仓/空仓动作必须齐全，A 股另需个股资金流；不达标时先触发自愈，仍未修复才暂停正式推送。
- [改进] 个股证据契约不达标时自动进入最多两轮自愈：按缺口清理股票级缓存、扩展日线窗口并遍历备用源、强制刷新财报/资金/资讯后重新分析复检，不再直接停留在 WebUI。
- [修复] 核心数据缺失链路硬化：美股新增与 Yahoo 熔断器解耦的 Stooq 免密钥日线/延时行情备援；A 股基本面改为核心财报和个股资金流优先消费超时预算，并新增东财财务指标备源；已落库的可追溯资讯可恢复为结构化事件证据；批量、合并和飞书推送统一执行正式报告完整性闸门。
- [修复] 复盘与外股证据链按云端失败日志修正：A 股市场宽度等待覆盖新浪全量回退的真实耗时；美股增加 Nasdaq/Yahoo 可追溯资讯备援；铠侠与智谱直连公司官方 IR 新闻页；外股财报日期可从 Yahoo 明确报告时间戳恢复；自愈仅重抓缺失模块并在连续无进展时停止重复循环，失败摘要保留准确原因。
- [修复] 个股行情增加跨字段算术校验：涨跌幅统一由报告中的收盘/前收盘复算，实时源缺少 OHLC 时优先保留同日完整日线，OHLC 关系或涨跌幅不一致会触发自愈而不进入正式推送；美股、港股、日股的价格提示、行情成交额和财务金额改用各自币种，并以“万亿”优化大额可读性。
- [修复] 盘中手动分析的行情表头不再误写为“收盘”；统一标记为“最新行情/盘中估算价”，并明示价格、涨跌幅和成交量尚未收盘。
- [修复] 持仓语义护栏拦截“空仓者卖出/减仓/清仓”等不可执行模型文本；看空时空仓者统一为保持空仓并等待风险解除，持仓者的减仓/止损建议保留。
- [改进] 每日预测复核与校准仍在后台自动执行并持久化，但默认不再另外推送校准摘要；微信仅保留大盘与个股决策报告，可通过 `ADAPTIVE_LEARNING_NOTIFY_ENABLED=true` 显式恢复。
