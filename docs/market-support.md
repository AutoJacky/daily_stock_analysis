...
美股大盘复盘沿用 A 股版的“事实采集、交易日对齐、结构校验、新闻核验、确定性评分、次日计划、缺失降级”方法，但不会把 A 股制度字段生搬到美股：

- 主要指数：标普500、纳斯达克综合、纳斯达克100、道琼斯、罗素2000和 VIX；至少 3 个非 VIX 指数必须落在同一交易日。
- 市场参与度：用 SPY、RSP、IWM、QQQ 的相对涨跌衡量市值权重、等权、小盘和科技成长是否共振；这是透明代理指标，不表述为交易所涨跌家数。
- 行业轮动：覆盖 GICS 对应的 11 个标普行业 ETF（XLK/XLC/XLY/XLP/XLE/XLF/XLV/XLI/XLB/XLRE/XLU），至少 8 个同交易日有效才允许生成方向结论。
- 流动性：使用 SPY 当日成交量相对前 20 个有效交易日均量，不使用 Yahoo 的估算成交额冒充全市场成交额。
- 宏观定价：优先通过 FRED 的 DGS2、DGS10 和 DTWEXBGS 读取美国 2 年/10 年期国债收益率与广义美元指数；若 FRED 美债序列不可达，则从美国财政部官方 Daily Treasury Par Yield Curve 补足 2 年/10 年收益率。每个字段均保留数据日、变化和来源，备用源不会读取晚于报告交易日的数据。
- 政策、政治与业绩：新闻检索覆盖美联储、财政、监管、关税、地缘政治、宏观数据、标普500财报与管理层指引；报告必须区分行情事实、带来源新闻与分析推断，缺少证据时写明“无法验证”。
- 质量闸门：指数与 ETF 交易日、参与度、SPY 量比、行业覆盖、2 年/10 年美债任一核心层失败时，跳过 LLM 方向解读并禁止输出仓位或交易计划。
- 事实审计：美股大盘正文由确定性模板渲染，LLM 不得改写行情、补写事件、生成支撑压力位或虚构行业上涨原因。政策、政治与财报只列带来源线索；没有检索证据时明确写“无法验证”。

GitHub Actions 使用两条收盘任务：北京时间 15:10 的完整任务只复盘 A 股；`21:15 UTC` 的独立任务只运行美股大盘复盘。后者对应美东冬令时收盘后 15 分钟、夏令时收盘后 75 分钟，以完整日线优先于抢跑。交易日检查按 `XNYS / America/New_York` 执行，美股休市日不会发送旧数据冒充当日复盘。

数据边界：

- Yahoo Finance/yfinance 属于延迟或非交易所直连行情，系统通过同日对齐和 fail-closed 降低错误风险，但不承诺交易所级实时行情 SLA。
- FRED 日频数据可能晚于美股收盘更新；报告显示每个宏观字段自己的数据日，不把滞后值写成盘中实时值。
- 本能力是研究和风控辅助，不执行自动下单；评分、仓位区间和历史校准不构成投资建议。

回滚方式：回退本次提交；或移除 GitHub Actions 的 `21:15 UTC` cron，并将 `MARKET_REVIEW_REGION` 恢复为 `cn`。
