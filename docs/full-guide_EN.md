...
> - If the official Shenwan level-1 endpoint is unavailable, the CN close review applies a 15-second hard timeout and may build an explicitly labelled proxy from Sina's complete 31-node SW2021 level-1 constituent tree. Returns are weighted by prior-session free-float market capitalization. The proxy passes only with all 31 industries, at least 80% valid constituent coverage per industry, and one aligned proven quote date; it is never presented as an official Shenwan index return.
