...
| Secret 名称 | 说明 | 必填 |
|------------|------|:----:|
| `STOCK_LIST` | 自选股代码，如 `600519,300750,002594,7203.T,285A.T,005930.KS`；日股支持旧数字代码和 JPX 2024 年起含字母的四位代码。推荐使用英文逗号，中文逗号、顿号、分号、空格和换行会被识别并规范为英文逗号 | ✅ |
...
| `FINNHUB_API_KEY` | Finnhub 美股行情凭据；实际可用能力取决于当前套餐，失败时自动换源 | 可选 |
| `ALPHAVANTAGE_API_KEY` | Alpha Vantage 美股行情凭据；需注意自身请求额度，失败时自动换源 | 可选 |
...
> **美股 Actions 备源：** `00-daily-analysis.yml` 已显式映射 `FINNHUB_API_KEY` 和 `ALPHAVANTAGE_API_KEY`；即使两者都未配置，个股日线与最新交易日行情仍会尝试免密钥 Stooq 备援。
...
> - 申万官方一级行业指数接口不可用时，A 股收盘复盘会在 15 秒硬超时后尝试新浪公开的完整 31 个申万2021一级行业节点，按同一交易日成分股的前一日流通市值加权收益生成“代理排名”。只有 31/31 行业齐全、每行业有效成分覆盖率不低于80%且代表行情交易日完全一致才会通过；报告会显式标注代理口径，不把代理涨跌幅冒充申万官方指数。
...
> - 美股大盘复盘采用同一事实优先流程，但按美股制度使用等价指标：6 个指数（含罗素2000/VIX）、SPY/RSP/IWM/QQQ 相对强弱、11 个标普行业 ETF、SPY/20日均量、FRED 2年/10年美债与广义美元指数，以及带来源的联储/财政/监管/关税/政治地缘/财报指引新闻。FRED 美债不可达时仅使用美国财政部官方收益率曲线备用源。核心层必须同交易日且完整通过，否则跳过 LLM 并禁止方向和仓位结论。详细口径见 [市场支持与边界](market-support.md#美股大盘严格复盘)。
> - 字段契约：
...
仓库默认安排三个收盘批次：A 股批次在北京时间 15:10 后先推送 A 股大盘复盘，再推送自选中 A 股的行动清单；港股/日股批次在北京时间 16:15 只推送对应个股行动清单；美股批次在 21:15 UTC 先推送美股大盘复盘，再推送自选中美股的行动清单。三个批次共用一份 `STOCK_LIST`，由 workflow 通过 `STOCK_MARKET_FILTER` 自动分流，不需要维护多份自选。
...
- 免费，无需配置
- 支持美股/港股数据
- 美股历史数据优先使用 YFinance；仍不使用 akshare 美股前复权日线，避免复权异常导致技术指标错误

### Stooq
- 免费、无需配置，提供美股个股日线及最新交易日延时行情备援
- 作为独立 fetcher 注册，Yahoo / yfinance 被限流、区域阻断或熔断时仍会被管理器尝试
- 报告会保留 Stooq 来源和延时属性，不冒充盘中实时行情
...
| `ADAPTIVE_LEARNING_ENABLED` | `false` | 保存每日模型治理快照，并在下一轮按客观表现自动收紧置信度；云端每日 workflow 默认开启 |

### 自动运行与每日校准闭环

每日完整分析会先推送大盘复盘和个股决策摘要，再执行一次客观历史复核；当天产生和刚到期的后验结果写入持久化数据库，供下一次定时分析读取。这样历史计算不会延迟最重要的收盘报告，失败也不会阻断大盘、个股或通知。

除默认 10 日窗口回测外，每日任务还会对已保存的 DecisionSignal 执行 1/3/5/10 个交易日后验。评分只比较已保存结论与真实后续行情，LLM 不参与给自己打分。逐股票完成样本少于 30 时只展示统计、不调整置信度；达到门槛后也只允许根据客观历史下调，绝不因历史胜率自动上调。数据、财报或公告证据缺失时继续由报告护栏降低置信度。

GitHub hosted runner 本身是临时环境，因此仓库自带的每日 workflow 会通过 Actions cache 恢复并保存 `data/stock_analysis.db`，让历史样本跨日连续积累，同时启用 `AGENT_MEMORY_ENABLED=true`。正式定时运行与手动验收使用隔离的缓存命名空间；只有定时运行且 SQLite 完整性检查通过后才保存，避免单股试跑污染正式样本。该机制只做置信度校准和保守降级，不承诺收益或固定命中率。也可通过 API 手动触发回测。

开启 `ADAPTIVE_LEARNING_ENABLED` 后，每日后验会额外写入版本化治理快照。状态固定为“采样积累 / 数据阻断 / 自动限制 / 谨慎校准 / 稳定观察”之一：少于 30 个完成样本不调整；数据不可评估率过高、方向命中率低或模拟收益明显为负时，下一轮自动把全局置信度系数压到 `0.65` 或 `0.85`。统计改善时最多恢复到 `1.0`，不会自动提高到原始置信度以上。

保守、均衡、进取三个 profile 只有在至少 60 个完成后验、方向命中率不低于 55%、不可评估率不高于 15% 时，才可能成为“影子冠军”；该状态只表示继续模拟验证，不会自动切换用户 profile。治理快照的 `live_trading_allowed` 永久为 `false`，代码不调用券商下单、改单、撤单或交易解锁接口。真实程序化交易需要用户另行取得券商量化终端/API权限、完成监管和券商要求的报告或协议、建立人工总开关及独立风险限额后再开发。
...
| 日股 | Yahoo 后缀 `.T`（支持 JPX 字母代码） | `7203.T`、`285A.T` |
