...
- 美股大盘复盘：YFinance 提供同日指数、ETF参与度/行业轮动和 SPY 量比，FRED 提供美债与美元日频锚；若 FRED 美债序列不可达，使用美国财政部官方每日收益率曲线补足 2 年/10 年收益率。核心证据层仍缺失时 fail-closed，不生成方向或仓位建议。
...
| 美股大盘复盘 | YFinance、FRED、美国财政部、既有新闻检索源 | 指数与 ETF 必须同交易日；DGS2/DGS10（FRED，失败时美国财政部官方曲线）为必需宏观层，DTWEXBGS 为增强层 | 任一必需层缺失则输出数据异常页并禁止 LLM 方向/仓位结论；不会用 0 或未来日期数据补位 |
