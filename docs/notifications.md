...
- 个股正式推送执行最低证据契约：股票身份、当日 OHLC/前收盘/成交量、MA5/10/20、最新报告期与至少两项核心财务指标、带日期/来源/链接的可核验事件、持仓/空仓动作建议必须齐全；A 股还要求个股资金流。不达标时不再直接停在 WebUI：默认立即按缺失类型清理该股缓存、将日线窗口扩大到 120 天后遍历备用源、强制刷新财报/资金与资讯缓存，然后重新分析并复检。当次默认最多 2 轮，仍失败才会保留可审计缺口并由下次定时/手动运行继续重试；始终不用 LLM 猜测缺失数据。可用 `PUSH_REPORT_SELF_HEAL_ENABLED=false` 关闭自愈，或用 `PUSH_REPORT_MINIMUM_DATA_ENABLED=false` 恢复宽松模式。
